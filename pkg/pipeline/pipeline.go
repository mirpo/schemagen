...
func validateConfig(cfg *Config) error {
	...
	if len(cfg.Schemas) == 0 {
		return &ValidationError{Field: "schemas", Message: "no schemas provided"}
	}
	if cfg.OutDir == "" {
		return &ValidationError{Field: "output-directory", Message: "output directory is empty"}
	}
	if cfg.Language == "" {
		return &ValidationError{Field: "language", Message: "language not specified"}
	}
